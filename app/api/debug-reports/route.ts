...
export async function GET(req: NextRequest) {
  ...
  const [latest, inRange, counts, itemCheck] = await Promise.all([
    // 5 đơn mới nhất của cửa hàng này
    sql`SELECT id, order_code, status, is_paid, total_amount, created_at FROM orders WHERE store_id = ${storeId} ORDER BY created_at DESC LIMIT 5`,
