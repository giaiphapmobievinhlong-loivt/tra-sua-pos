  ...
  try {
      ...
      const [stats, thuChi, daily, top_products, trend] = await Promise.all([
        sql`
          ...
          WHERE store_id = ${user.store_id}
            AND created_at >= ${monthStart}::timestamptz AND created_at < ${nextMonth}::timestamptz
        ...
        sql`
          ...
          WHERE o.store_id = ${user.store_id}
            AND o.created_at >= ${monthStart}::timestamptz AND o.created_at < ${nextMonth}::timestamptz
        ...
        sql`
          ...
          FROM orders
          WHERE store_id = ${user.store_id}
            AND created_at >= (${monthStart}::timestamptz - interval '5 months')
    ...
    const [stats, thuChi, recent_orders, hourly, cups, top_products] = await Promise.all([
      sql`
        ...
        FROM orders
        WHERE store_id = ${user.store_id}
          AND created_at >= ${dayStart}::timestamptz AND created_at < ${dayEnd}::timestamptz
          AND status != 'cancelled' AND is_paid = true
      `,
      sql`
        SELECT COALESCE(SUM(CASE WHEN type='thu' THEN amount ELSE 0 END),0) as total_thu,
               COALESCE(SUM(CASE WHEN type='chi' THEN amount ELSE 0 END),0) as total_chi
        FROM transactions WHERE store_id = ${user.store_id} AND transaction_date = ${date}
      `,
      sql`
        SELECT o.*, TO_CHAR(o.created_at + interval '7 hours', 'YYYY-MM-DD HH24:MI:SS') as vn_created_at, u.username
        FROM orders o LEFT JOIN users u ON o.user_id = u.id
        WHERE o.store_id = ${user.store_id}
          AND o.created_at >= ${dayStart}::timestamptz AND o.created_at < ${dayEnd}::timestamptz
