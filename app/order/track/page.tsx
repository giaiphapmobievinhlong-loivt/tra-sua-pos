...
function TrackContent() {
  ...
  async function fetchOrder(c: string) {
    ...
    try {
      ...
      if (data.order) { setOrder(data.order); setLastUpdated(new Date()) }
      else setError(data.error || 'Không tìm thấy đơn')
    } catch { setError('Lỗi kết nối') }
    finally { setLoading(false) }
  }

  // Auto-fetch on load and every 10s if order not completed
  useEffect(() => {
      ...
      <div className="max-w-md mx-auto px-4 py-6">
        ...
        {order && (
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-orange-100">
              ...
              <div className="flex gap-2 flex-wrap">
                {order.table_number
                  ...
                  : <span className="text-xs bg-gray-100 text-gray-600 font-bold px-2.5 py-1 rounded-full">🛍 Mang về</span>}
            ...
            {/* Items */}
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-orange-100">
              <p className="font-bold text-gray-800 mb-3 text-sm">Chi tiết đơn</p>
                ...
                {order.items.map((item, i) => (
                  <div key={i}>
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-bold tabular-nums">{fmt(Number(item.subtotal))}đ</span>
                    </div>
