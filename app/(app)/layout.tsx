...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const [store] = await sql`SELECT name FROM stores WHERE id = ${user.store_id}`

  return (
    <div className="flex h-screen bg-[#f8f5f0]">
      <Sidebar username={user.full_name || user.username} role={user.role} storeName={store?.name || 'Cửa hàng'} />
